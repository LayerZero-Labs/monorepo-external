...
import type { IWriteMarkDown } from './contracts';
import {
    addCommandLineArgsFooter,
    addContent,
    generateUsageGuides,
    insertCode,
} from './helpers/index';
...
async function writeMarkdown() {
    ...
    modifiedFileContent = await insertCode(
        { fileContent: modifiedFileContent, filePath: markdownPath },
        args,
    );
        ...
        case 'verify_nonMatch':
            console.warn(
                chalk.yellow(
                    format(
                        args.verifyMessage ||
                            `'${relativePath}' file out of date. Rerun write-markdown to update.`,
                        {
