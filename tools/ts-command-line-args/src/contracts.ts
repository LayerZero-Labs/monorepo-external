...
export type PropertyConfig<T> = undefined extends T
    ? PropertyOptions<T>
    : RequiredPropertyOptions<T>;
export type RequiredPropertyOptions<T> =
    Array<any> extends T ? PropertyOptions<T> : TypeConstructor<T> | PropertyOptions<T>;

export type TypeConstructor<T> = (
    value: any,
) => T extends Array<infer R> ? R | undefined : T | undefined;
...
export type MultiplePropertyOptions<T> =
    Array<any> extends T ? { multiple: true } | { lazyMultiple: true } : unknown;

export type HeaderLevel = 1 | 2 | 3 | 4 | 5;

export type PickType<T, TType> = Pick<
    T,
    { [K in keyof T]: Required<T>[K] extends TType ? K : never }[keyof T]
>;
...
export interface ArgsParseOptions<T> extends UsageGuideOptions {
    ...
    /**
     * If true, arguments will be matched regardless of their casing. E.g., argument is defined myParam,
     * user provides --myparam 'xyz'. Return will have args.myParam === 'xyz'
     */
