...
export function createUsageGuide<T = any>(config: UsageGuideConfig<T>): string {
    ...
    return [
        ...headerSections
            .filter(filterMarkdownSections)
            .map((section) => createSection(section, config)),
...
function filterMarkdownSections(section: Content): boolean {
    return (
        section.includeIn == null ||
        section.includeIn === 'both' ||
        section.includeIn === 'markdown'
    );
}
...
export function createOptionsSections<T>(
    cliArguments: ArgumentConfig<T>,
    ...
    return getOptionSections(options).map((section) =>
        createOptionsSection(optionList, section, options),
    );
}
...
function filterOptions(option: CommandLineOption, groups?: string | string[]): boolean {
    return (
        ...
        (typeof groups === 'string' &&
            (groups === option.group || (groups === '_none' && option.group == null))) ||
...
export function createOptionRow(
    ...
    const alias = includeAlias
        ? ` ${option.alias == null ? '' : '**' + option.alias + '** '}|`
        : ``;
...
export async function loadArgConfig(
    jsFile: string,
    importName: string,
): Promise<UsageGuideConfig | undefined> {
    ...
    if (argConfig == null) {
        console.warn(
            `Could not import ArgumentConfig named '${importName}' from jsFile '${jsFile}'`,
        );
        return undefined;
    }
