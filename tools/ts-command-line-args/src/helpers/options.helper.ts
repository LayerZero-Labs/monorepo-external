...
export function getOptionFooterSection<T>(
...
export function generateTableFooter<T>(
    ...
    if (
        options.addOptionalDefaultExplanatoryFooter != true ||
        options.displayOptionalAndDefault != true
    ) {
        return undefined;
    }

    const optionalProps = optionList.some(
        (option) => (option as unknown as OptionalProperty).optional === true,
    );
...
function mapOptionDescription<T>(
    definition: CommandLineOption<T>,
    options: ParseOptions<T>,
): CommandLineOption<T> {
