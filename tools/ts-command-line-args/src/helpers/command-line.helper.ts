...
export function createCommandLineConfig<T>(config: ArgumentOptions<T>): CommandLineOption[] {
    return Object.keys(config).map((key) => {
        ...
        const definition: PropertyOptions<any> =
            typeof argConfig === 'object' ? argConfig : { type: argConfig };
...
export function mergeConfig<T>(
    ...
    return {
        ...parsedConfig,
        ...applyTypeConversion(configFromFile, options),
        ...parsedConfigWithoutDefaults,
    };
}

function resolveConfigFromFile<T>(
    configfromFile: any,
    configPath?: string,
...
export function removeBooleanValues<T>(args: string[], config: ArgumentOptions<T>): string[] {
    function removeBooleanArgs(
        argsAndLastValue: ArgsAndLastOption,
        arg: string,
    ): ArgsAndLastOption {
...
export function getBooleanValues<T>(args: string[], config: ArgumentOptions<T>): Partial<T> {
    function getBooleanValues(
        argsAndLastOption: PartialAndLastOption<T>,
        arg: string,
    ): PartialAndLastOption<T> {
...
function getParamConfig<T>(
    ...
    for (const argName in config) {
        ...
        if (argName === nameOrAlias || argConfig.alias === nameOrAlias) {
            return {
                argOptions: argConfig as PropertyOptions<any>,
                argName,
