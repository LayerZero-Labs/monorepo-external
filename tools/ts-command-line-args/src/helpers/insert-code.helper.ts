...
import type { IInsertCodeOptions } from '../contracts';
...
async function insertCodeImpl(
    ...
    const insertCodeAboveResult =
        insertCodeAbove != null
            ? findIndex(
                  lines,
                  (line) => line.indexOf(insertCodeAbove) === 0,
                  insertCodeBelowResult.lineIndex,
              )
            : undefined;
    ...
    const linesAfter =
        insertCodeAboveResult != null ? lines.slice(insertCodeAboveResult.lineIndex) : [];
...
async function loadLines(
    ...
    const filePath = isAbsolute(partialPath)
        ? partialPath
        : join(dirname(targetFilePath), partialPathResult[1]);
    console.log(
        `Inserting code from '${chalk.blue(filePath)}' into '${chalk.blue(targetFilePath)}'`,
    );
    ...
    const copyBelowIndex =
        copyBelowMarker != null
            ? contentLines.findIndex(findLine(copyBelowMarker, snippetResult?.[1]))
            : -1;
    const copyAboveIndex =
        copyAboveMarker != null
            ? contentLines.findIndex(
                  (line, index) => line.indexOf(copyAboveMarker) === 0 && index > copyBelowIndex,
              )
            : -1;
    ...
    contentLines = contentLines.slice(
        copyBelowIndex + 1,
        copyAboveIndex > 0 ? copyAboveIndex : undefined,
    );
...
function findLine(copyBelowMarker: string, snippetName?: string): (line: string) => boolean {
    return (line: string): boolean => {
        return (
            line.indexOf(copyBelowMarker) === 0 &&
            (snippetName == null || line.indexOf(snippetName) > 0)
        );
    };
}
