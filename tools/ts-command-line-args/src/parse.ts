...
export function parse<T, P extends ParseOptions<T> = ParseOptions<T>, R extends boolean = false>(
    ...
    if (options.helpArg != null && (parsedArgs as any)[options.helpArg]) {
        ...
        if (options.showHelpWhenArgsMissing) {
            ...
            const additionalHeaderSections: Content[] =
                missingArgsHeader != null ? [missingArgsHeader] : [];
            ...
            printUsageGuideMessage(
                { ...options, logger },
                options.helpArg != null
                    ? optionList.filter((option) => option.name === options.helpArg)[0]
                    : undefined,
            );
        }
    }
...
function printMissingArgErrors(
    ...
    missingArgs.forEach((config) => {
        const aliasMessage =
            config.alias != null ? ` or '${baseCommand}-${config.alias} passedValue'` : ``;
        const runCommand =
            baseCommand !== ''
        ...
        logger.error(
            `Required parameter '${config.name}' was not passed. Please provide a value by ${runCommand}`,
        );
    });
}

function printUsageGuideMessage(
    options: UsageGuideOptions & { logger: typeof console },
    helpParam?: CommandLineOption,
) {
    if (helpParam != null) {
        const helpArg = helpParam.alias != null ? `-${helpParam.alias}` : `--${helpParam.name}`;
        const command =
            options.baseCommand != null
                ? `run '${options.baseCommand} ${helpArg}'`
                : `pass '${helpArg}'`;
...
function listMissingArgs(
    commandLineConfig: CommandLineOption[],
    parsedArgs: commandLineArgs.CommandLineOptions,
) {
