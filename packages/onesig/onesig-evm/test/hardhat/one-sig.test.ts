...
describe('OneSig', () => {
    ...
    it('executes multiple transactions', async () => {
        ...
        const evmGen = evmLeafGenerator([
            // eth transactions
            {
                ...
                calls: [
                    transferEtherCall(recipientA, ONE_ETHER),
                    transferEtherCall(recipientA, ONE_ETHER),
                ],
            },
            {
        ...
        const expiry = await ethers.provider
            .getBlock('latest')
    ...
    it('replay protection', async () => {
        ...
        await oneSig
            .connect(executor1)
            .executeTransaction(tx, tree.tree.getRoot(), tree.signingData.expiry, signatures);
    ...
    it('invalid signatures', async () => {
        ...
        await expect(
            oneSig
                .connect(executor1)
                .executeTransaction(tx, tree.tree.getRoot(), tree.signingData.expiry, '0x11112222'),
        ).to.be.revertedWithCustomError(oneSig, 'SignatureError');

        // Sign the tree by fake signers
        const fakeSigners = [signers[2], signers[3]].sort((a, b) =>
        ...
        const combinedSignature = Signature.concatenateSignatures(
            singleSignatures.reverse(),
            false,
        );
    ...
    it('executors test', async () => {
        ...
        await expect(
            (await makeSetExecutorCall(executor2.address, true))(),
        ).to.be.revertedWithCustomError(oneSig, 'ExecutionFailed');
        ...
        await expect(oneSig.setExecutorRequired(false)).to.be.revertedWithCustomError(
            oneSig,
            'OnlySelfCall',
        );
        ...
        await (
            await makeOneSigCall(
                oneSig.interface.encodeFunctionData('setExecutorRequired', [false]),
                3n,
            )
        )();
    ...
    it('profile', async () => {
        ...
        const calls = Array.from({ length: numberOfCalls }, (_, i) =>
            setBytesCall(mockOApp.address, i, bytesToSet),
        );
